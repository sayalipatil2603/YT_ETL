#!/bin/bash    #using the bash shell to run the script
...
function create_user_and_database() {
    ...
    psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL #psql is the command line tool to interact with postgreSQL databases
