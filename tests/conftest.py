...
def api_key():
    with mock.patch.dict("os.environ", AIRFLOW_VAR_API_KEY="MOCK_KEY1234"):
        yield Variable.get("API_KEY") #Variable.get() fetches te value of any airflow variable, yeild gives the value to the test
...
def mock_postgres_conn_vars():
    ...
    conn_uri = conn.get_uri() #Converts the connection into an Airflow connection URI
...
def airflow_variable():
    def get_airflow_variable(variable_name):
        env_var = f"AIRFLOW_VAR_{variable_name.upper()}" #Converts the variable name into the Airflow environment variable format.
...
def real_postgres_connection():
    dbname = os.getenv("ELT_DATABASE_NAME") #os.getenv is a Python function used to read environment variables.
