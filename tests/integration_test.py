...
def test_youtube_api_response(airflow_variable):
    api_key = airflow_variable("api_key") #Pytest injects the fixture, so airflow_variable is actually the function get_airflow_variable.   
...
def test_real_postgres_connection(real_postgres_connection):
    ...
    try:
        ...
        result = cursor.fetchone() #because result is a tuple when created using the cursor method

        assert result[0] == 1 #first element in the result is 1
