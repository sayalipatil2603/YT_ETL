def test_api_key(api_key):
    assert api_key == "MOCK_KEY1234" #to verify the API key
...
def test_dags_integrity(dagbag):
    ...
    # 2.
    expected_dag_ids = ["produce_json", "update_json", "data_quality"]
    loaded_dag_ids = list(dagbag.dags.keys()) #dag_id 
    print("===========")
    ...
    for dag_id, dag in dagbag.dags.items():
        ...
        actual_count = len(dag.tasks) #len() counts the number of tasks in the dag
        assert (
