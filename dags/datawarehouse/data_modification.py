...
def insert_rows(cur, conn, schema, row):
    ...
    try:
        ...
        if schema == 'staging':
            ...
            #the variable names will be based on json keys
            cur.execute(
                f"""
            ...
            #Pyhton placeholders 
            #row is a dictionary which contains the values we want to insert

        else:

            video_id = "Video_ID"

            #now we are reading data from staging layer so core layer names should match with that of staging   
            cur.execute(
                f"""
...
def update_rows(cur, conn, schema, row):
    ...
    try:
        ...
        logger.error(f"Error updating row with Video_ID: {row[video_id]} - {e}") #e displays the error message
        raise e
...
def delete_rows(cur, conn, schema, ids_to_delete):
    ...
    try:
        ...
        ids_to_delete = f"""({', '.join(f"'{id}'" for id in ids_to_delete)})""" #gives comma separated values
